...
case class LinkCtrl(master: Boolean, pri: Boolean, fcb: Boolean, fcvdfc: Boolean, func: Byte) extends SerializableToBytes {
  ...
  def modFCV(fcv: Boolean) = this.copy(fcvdfc = fcv)
...
case class LinkHeader(len: UInt8, ctrl: LinkCtrl, dest: UInt16LE, src: UInt16LE) extends SerializableToBytes {
  ...
  def bytesNoSync = len ++ ctrl ++ dest ++ src

  def toBytesWithGen(gen: CrcGenerator) = gen.append(bytes)
...
case class Lpdu(header: LinkHeader, data: List[Byte]) extends SerializableToBytes {
  ...
  def toBytesWithGen(headerCrc: CrcGenerator, bodyCrc: CrcGenerator) = header.toBytesWithGen(headerCrc) ++ bodyCrc.interlace(data)

  def toBytesWithStart(first: Byte, second: Byte) = Crc.append(first :: second :: header.bytesNoSync) ++ Crc.interlace(data)
