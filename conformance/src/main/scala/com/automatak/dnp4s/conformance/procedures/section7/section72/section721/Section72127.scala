...
import com.automatak.dnp4s.dsl.{ UInt32LE, UInt8 }
...
object Section72127 extends TestProcedure with RequiresGroup12Var1Support {

  def id = "7.2.1.2.7"
  def description = "SBO, Select/Operate Mismatch (On Time)"
  ...
  def steps(options: TestOptions) = {
    ...
    val crob2 = crob.copy(onTime = UInt32LE(crob.onTime.i + 1))
  ...
  /*
7.2.1.2.7 SBO, Select/Operate Mismatch (On Time)
...
and that no error IINs are set. Note: The DUT fails this test if it rejects this request because
the timer in the prior test is still active.
c. Immediately issue an OPERATE request with Function Code 4 and the application sequence
number incremented by 1 (modulo 16), matching the SELECT message except that the on
