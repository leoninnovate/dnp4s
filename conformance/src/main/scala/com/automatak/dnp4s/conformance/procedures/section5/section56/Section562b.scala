...
import com.automatak.dnp4s.conformance.procedures.{ Prompts, RequiresSelfAddressSupport }

object Section562b extends TestProcedure with RequiresSelfAddressSupport {

  def id = "5.6.2b"
  def description = "Self-Address Support (steps 1g - 1h)"

  override def prompts = Prompts.disableSelfAddressSupport :: Prompts.cyclePower :: Nil

  def steps(options: TestOptions) = List(
    "1.g" -> List(Section56.RequestClass0WithAddress(UInt16LE(0xFFFC))),
    "1.h" -> List(LinkSteps.ExpectLpduTimeout))
