...
object Section721212 extends TestProcedure with RequiresGroup12Var1Support {

  def id = "7.2.1.2.12"
  def description = "BO, Select Retry (Same Sequence Number)"
  ...
  def steps(options: TestOptions) = {
    ...
    Operation.stepsSelectRetrySameSequenceNumber(header)
  }

  /*
7.2.1.2.12 SBO, Select Retry (Same Sequence Number)
...
c. Immediately issue the SELECT message again using the same Application Layer sequence
number.
d. Verify that the DUT echoes the SELECT message, that the status code is 0, and that no error
IINs are set.
...
and that no error IINs are set.
g. Verify that the selected control operates.
*/
}
