...
object Section74127 extends TestProcedure with RequiresAnalogOutputControlSupport {

  import AnalogOutputOptions._

  def id = "7.4.1.2.7"
  def description = "SBO, Select/Operate Mismatch (Value)"
  override def options = List(validIndex1, validValue1, validValue2)
  ...
  def steps(options: TestOptions) = {
    ...
    val header1 = TwoByteCountTwoByteIndexHeader(Group41Var2, UInt16LE(1), UInt16LE(index).toBytes ::: ao1.toBytes)
    val header2 = header1.copy(data = UInt16LE(index).toBytes ::: ao2.toBytes)
    val reply = header2.copy(data = UInt16LE(index).toBytes ::: ao2.copy(status = CommandStatus.NO_SELECT).toBytes)

    Operation.stepsBadSBO(header1, header2, reply)
  }

  /*
7.4.1.2.7 SBO, Select/Operate Mismatch (Value)
