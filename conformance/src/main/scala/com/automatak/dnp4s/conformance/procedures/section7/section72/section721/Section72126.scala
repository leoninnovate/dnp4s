...
object Section72126 extends TestProcedure with RequiresGroup12Var1Support {
  ...
  def steps(options: TestOptions) = {
    ...
    val headerOut1 = TwoByteCountTwoByteIndexHeader(Group12Var1, UInt16LE(1), UInt16LE(index1).toBytes ::: crob.toBytes)
    val headerOut2 = headerOut1.copy(data = UInt16LE(index2).toBytes ::: crob.toBytes)
    val headerFailure = headerOut2.copy(data = UInt16LE(index2).toBytes ::: crob.copy(status = CommandStatus.NO_SELECT).toBytes)

    Operation.stepsBadSBO(headerOut1, headerOut2, headerFailure)
  }

  /*
7.2.1.2.6 SBO, Select/Operate Mismatch (Point Number)
